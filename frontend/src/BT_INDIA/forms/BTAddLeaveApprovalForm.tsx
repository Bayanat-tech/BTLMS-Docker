...
import HRLeaveFilesDialog from 'pages/HR/HRFlow/HRFileAttachment/HRLeaveFilesDialog';
...
import BTFileUploadServiceInstance from '../service/services.BT_files';
...
`;
  ...
  const { data: attachedFiles, refetch: refetchAttachedFiles } = useQuery({
    ...
    queryFn: async () => {
      ...
      const response = await BTFileUploadServiceInstance.getEmployeeFiles(formData.request_number, 'hr');
      return response || [];
    },
  ...
  const normalizeToString = (val: any): string => {
    ...
    if (typeof val === 'object') return val.message || val.error || val.success || JSON.stringify(val);
    return String(val);
  };
  ...
  useEffect(() => {
    ...
    if (data) {
      console.log('Setting form data from props data', data);
      setFormData({
        request_number: normalizeToString(data.REQUEST_NUMBER),
        request_date: normalizeToString(data.REQUEST_DATE),
        employee_code: normalizeToString(data.EMPLOYEE_CODE),
        leave_type: normalizeToString(data.LEAVE_TYPE),
        leave_type_desc: normalizeToString(data.LEAVE_TYPE_DESC),
        Employee_Name: normalizeToString(data.EMPLOYEE_NAME),
        leave_start_date: normalizeToString(data.LEAVE_START_DATE),
        leave_end_date: normalizeToString(data.LEAVE_END_DATE),
        resume_date: normalizeToString(data.RESUME_DATE),
        leave_days: data.LEAVE_DAYS?.toString() || '',
        remarks: normalizeToString(data.REMARKS),
        company_code: user?.company_code || '',
        rpt_name: normalizeToString(data.EMPLOYEE_CODE),
        EMPLOYEE_ID: normalizeToString(data.EMPLOYEE_ID || data.EMPLOYEE_CODE),
        SUPERVISOR_EMPID: normalizeToString(data.IMMEDIATE_SUPERVISOR),
        DEPT_HEAD_EMPID: normalizeToString(data.DEPT_HEAD),
        MANGR_EMPID: normalizeToString(data.HOD),
        contact_details_during_leave: normalizeToString(data.CONTACT_DETAILS_DURING_LEAVE),
        CONTACT_DETAILS_DURING_LEAVE: normalizeToString(data.CONTACT_DETAILS_DURING_LEAVE),
        NAME_OF_REPLACEMENT: normalizeToString(data.NAME_OF_REPLACEMENT),
        TRAVEL_DATE: normalizeToString(data.TRAVEL_DATE),
        TRAVEL_END_DATE: normalizeToString(data.TRAVEL_END_DATE),
        CAUSE_TYPE: normalizeToString(data.CAUSE_TYPE),
        ADV_PAYMENT: normalizeToString(data.ADV_PAYMENT),
        LEAVE_ALLOWANCE: normalizeToString(data.LEAVE_ALLOWANCE),
        is_half_day: normalizeToString(data.HALF_DAY) || 'N',
        resume_work: data.RESUME_WORK || false,
        actual_resume_date: normalizeToString(data.ACTUAL_RESUME_DATE),
        DUTY_RESUME_DATE: normalizeToString(data.DUTY_RESUME_DATE),
        AIR_ROUTE: normalizeToString(data.AIR_ROUTE),
        AIR_TICKET: normalizeToString(data.AIR_TICKET),
        IMMEDIATE_SUPERVISOR_NAME: normalizeToString(data.IMMEDIATE_SUPERVISOR_NAME),
        HOD_NAME: normalizeToString(data.HOD_NAME),
        DEPT_HEAD_NAME: normalizeToString(data.DEPT_HEAD_NAME)
      });
      setFormReady(true);
    }
  ...
  useEffect(() => {
    if (data) {
      // Add this to find the culprit field
      console.log('RAW DATA FIELDS:');
      Object.entries(data).forEach(([key, value]) => {
        if (typeof value === 'object' && value !== null) {
          console.log(`OBJECT FIELD FOUND -> key: ${key}, value:`, value);
        }
      });
    }
  }, [data]);
    ...
    const fetchLeaveDays = async () => {
      ...
      if (formData.leave_start_date && formData.leave_end_date && formData.leave_type) {
        ...
        if (!isNaN(start.getTime()) && !isNaN(end.getTime())) {
          try {
            const result = await BTHrRequestServiceInstance.BTLeaveDaysCount({
  ...
  const fetchLeaveTypes = async (employeeId: string) => {
    ...
    try {
      ...
      const leaveHistory = await BTHrRequestServiceInstance.BTgetLeaveHistory({ employeeId });
    ...
    try {
      ...
      const normalizeMessage = (msg: any): string => {
        ...
        if (typeof msg === 'object') {
          return msg.message || msg.error || msg.success || JSON.stringify(msg);
        }
        return String(msg);
      };

      const normalizedMessage = normalizeMessage(result.message);

      let isValid = result.success && result.isValid;
      let message = normalizedMessage || 'Leave validation passed!';
      ...
      // Guard: only run split logic if message is a string containing '$$$'
      if (typeof normalizedMessage === 'string' && normalizedMessage.includes('$$$')) {
  ...
  const handleSave = async (actionType: string) => {
    ...
    const result = await dispatch(BTHrServiceInstance.upsertLeaveApprovalManualHandler(payload));
