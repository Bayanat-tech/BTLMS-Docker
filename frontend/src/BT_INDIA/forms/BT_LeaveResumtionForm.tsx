...
const BTLeaveResumptionForm: React.FC<AddLeaveApprovalFormProps> = ({
  data,
  onClose,
  onSuccess,
  isEditMode,
  ...
  const getEmployeeNameById = async (employeeId: string): Promise<string> => {
    ...
    try {
      const employeeArr = await BTHrRequestServiceInstance.getEmployees(employeeId);
  ...
  const fetchLeaveTypes = async (employeeId: string) => {
    ...
    try {
      const leaveHistory = await BTHrRequestServiceInstance.BTgetLeaveHistory({
