...
class HrRequestService {
  ...
  async BTgetLeaveHistory(params: ILeaveHistoryParams): Promise<ILeaveHistory[]> {
    try {
      ...
      const response = await axiosServices.get(`/api/hr/gm/BTleaveentitle/${employeeId}`, {
        params: queryParams
      });
    ...
    async BTLeaveDaysCount({
      leaveStartDate,
      leaveEndDate,
      leaveType
    }: ILeaveDaysParams) {
      const response = await axiosServices.get("/api/hr/gm/BTleavedayscount", {
        params: {
  ...
  async getValidateLeave(params: IValidateLeaveParams): Promise<IValidateLeaveResponse> {
    ...
    try {
      ...
      const response = await axiosServices.get('/api/hr/gm/BTvalidateleave', {
        params: queryParams,
        timeout: 30000
      });
