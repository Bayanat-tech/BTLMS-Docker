...
class HRService {
  ...
  upsertLeaveApprovalManualHandler =
    ...
    async (dispatch: AppDispatch): Promise<boolean> => {
      try {
        ...
        const response = await axiosServices.put<IApiResponse<any>>('api/HR/gm/BTupsertLeaveApprovalHandler', payload);
  ...
  getMasters = async (
    ...
    try {
      ...
      const shouldAddCode = masters === 'BT_Pg_Leave_flow' || masters !== 'BT_hrSection';

      console.log('Bt_hr', `api/${app_code}/bt/${masters}/${shouldAddCode ? `?code=${code}`: ''}`);

      const response: IApiResponse<{ tableData: unknown[]; count: number }> = await axiosServices.get(
        `api/${app_code}/bt/${masters}${shouldAddCode ? `?code=${code}` : ''}`,
        {
          params: {
            ...(page !== undefined && { page }),
