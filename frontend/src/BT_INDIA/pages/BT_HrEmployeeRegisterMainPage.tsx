...
const BT_HrEmployeeRegisterMainPage: React.FC<Props> = ({ leaveTypes = [], leaveData = [], onView = () => {}, isLoading = false }) => {
...
const { data: leaveBalance = [], isLoading: leaveBalanceLoading } = useQuery<ILeaveBalance[]>({
  ...
  const {
    ...
    queryFn: async () => {
      if (!employee) return [];
      try {
        const response = await BTHrRequestServiceInstance.BTgetLeaveHistory({
          employeeId: employee
        });
  ...
  const {
    ...
    queryFn: async () => {
      if (!employee) return [];
      try {
        const response = await BTHrServiceInstance.executeRawSql(filtered_leavestring);
        return response as unknown as ILeaveHistory[];
      } catch (err) {
