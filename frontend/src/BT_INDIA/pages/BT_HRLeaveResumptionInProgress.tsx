...
const BTHRLeaveResumptionInProgress: FC<HRLInProgressProps> = ({}) => {
  const intl = useIntl();
  ...
  const { data: HRLInProgressData } = useQuery({
    ...
    queryFn: () => BTHrServiceInstance.getMasters('hr', 'BT_Pg_leave_flow_InProgress', paginationData, searchData, user?.loginid1),
...
`;
  ...
  const {
    ...
    queryKey: ['BT_Pg_Leave_flow', paginationData, user?.loginid1, sql_string],
    queryFn: () => BTHrServiceInstance.executeRawSql(sql_string),
  ...
  const { data: editData } = useQuery({
    ...
    queryFn: () =>
      selectedRequestNumber
        ? BTHrServiceInstance.getMasters('hr', 'BT_Leaveflow_request', undefined, undefined, selectedRequestNumber)
...
export default BTHRLeaveResumptionInProgress;
